...
public class ModelBasedEvaluation {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		String path = String.format("../org.mondo.collaboration.security.generator/instances/model-%04d-%04d-%04d.xmi".replace('/', File.separatorChar), Configuration.MODEL_SIZE, Configuration.DEEP_SIZE, Configuration.VENDOR_SIZE);
		...
		for(int i = 1; i <= Configuration.TypeA; i++) {
			UserType user = new UserTypeAMBL(resource, "cycle."+(i%Configuration.VENDOR_SIZE), locker).init();
			user.setName("userA"+i);
			user.start();
			users.add(user);
		}
		for(int i = 1; i <= Configuration.TypeB; i++) {
			UserType user = new UserTypeBMBL(resource, "type."+(i%Configuration.VENDOR_SIZE), locker).init();
			user.setName("userB"+i);
			user.start();
			users.add(user);
		}
		for(int i = 1; i <= Configuration.TypeC; i++) {
			UserType user = new UserTypeBMBL(resource, "vendor."+(i%Configuration.VENDOR_SIZE), locker).init();
